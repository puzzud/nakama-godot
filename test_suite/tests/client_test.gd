...
func setup():
	...
	if assert_cond(session.is_valid()) or assert_cond(!session.is_expired()):
		return
	# PUT
	var update = await client.update_account_async(session, username).completed
	if assert_false(update.is_exception()):
		return
	# GET
	var account : NakamaAPI.ApiAccount = await client.get_account_async(session).completed
	if assert_false(account.is_exception()):
		return
	if assert_cond(account.user.username == username):
		return
	# POST - DELETE
	var group : NakamaAPI.ApiGroup = await client.create_group_async(session, "MyGroupName3").completed
	if assert_false(group.is_exception()):
		return
	var delete = await client.delete_group_async(session, group.id).completed
