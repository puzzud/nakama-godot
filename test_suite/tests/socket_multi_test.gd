...
func setup():
	...
	socket1.connect("received_channel_message", Callable(self, "_on_socket1_message"))
	socket1.connect("received_matchmaker_matched", Callable(self, "_on_socket1_matchmaker_matched"))
	...
	done = await socket2.connect_async(session2).completed
	# Check that connection succeded
	if assert_false(done.is_exception()):
		return

	# Join room
	var room1 = await socket1.join_chat_async("MyRoom", NakamaSocket.ChannelType.Room).completed
	if assert_false(room1.is_exception()):
		return
	var room2 = await socket2.join_chat_async("MyRoom", NakamaSocket.ChannelType.Room).completed
	if assert_false(room2.is_exception()):
		return

	# Socket 2 send message to socket 1
	var msg_ack = await socket2.write_chat_message_async(room2.id, content).completed
	if assert_false(msg_ack.is_exception()):
		return

	var ticket1 = await socket1.add_matchmaker_async("+properties.region:europe +properties.rank:>=7 +properties.rank:<=9", 2, 8, match_string_props, match_numeric_props).completed
	if assert_false(ticket1.is_exception()):
		return
	var ticket2 = await socket2.add_matchmaker_async("+properties.region:europe +properties.rank:>=7 +properties.rank:<=9", 2, 8, match_string_props, match_numeric_props).completed
